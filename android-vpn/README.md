...
```
приложения телефона -> tun0 (10.80.1.<n>) -> libhomeproxy.so (10 дыр, keep-alive)
...
2. **«Включить VPN»** доступна, только когда есть хотя бы одна живая дыра
   (`HomeProxy.liveHoles() >= 1`); автозапуск ждёт их до 90 секунд. `HpVpnService` поднимает
   интерфейс: адрес `10.80.1.<n>/32`, маршрут на всё, DNS, MTU 1400, **само приложение
...
Адрес в туннеле `10.80.1.<n>`: за роутером `n` — номер телефона в настройках роутера
(`PHONE_<n>_*`), по адресу VPS знает, какому телефону отвечать. Для схемы с ПК — любой адрес из
подсети `TUN_ADDR` сервера, кроме адреса самого ПК.

В логе (`logcat -s homeproxy`):

```
слот 0: дыра открыта … (живых дыр: 1)
VPN включён: 10.80.1.1, DNS 1.1.1.1
состояние: дыры 10/10, TUN подключён, отправлено 1208, получено 1008, потеряно 0
```
...
adb shell am start -n ru.homeproxy/.MainActivity \
  ...
  --es tunAddr 10.80.1.1 --es dns 1.1.1.1 \
