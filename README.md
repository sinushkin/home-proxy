...
flowchart LR
    ...
    subgraph home["Дом: за NAT домашнего роутера"]
        ...
        wg2 --> net(("Интернет: ваш домашний IP"))
    end
...
- **Симметричный NAT** (порт зависит от адресата), как у части операторов и точек доступа,
  пробить обычным способом нельзя. Как раз это направление ждёт помощи.
...
Запросы на слияние (MR/PR) приветствуются: пробив через симметричный NAT, внутрисетевые
кандидаты (когда оба за одним адресом), клиент для iOS, замена XOR-маскировки на
аутентифицированную обёртку, проверка на реальных устройствах.
