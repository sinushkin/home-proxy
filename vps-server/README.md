...
| `TUN_NAME`, `TUN_MTU`, `ADDRESS_FILE`, `DNS`, `REORDER_WAIT_MS`, `DATA_HOLES`, `RUST_LOG`, `LOG_FILE` | как у `hp-server` |
...
клиента, со своими номерами. Адреса раздаёт сам сервер: роутеру — из `10.80.0.x`, его
телефонам (запросы пересылает роутер с их номером) — из `10.80.1.x` и дальше; вместе с адресом —
DNS. Пакет с адресом источника, выданным не этому клиенту, отбрасывается. Настройка целиком —
`../OpenWRT/Tun.md`.
