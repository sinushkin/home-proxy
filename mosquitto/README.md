...
```
home-proxy/rendezvous/<имя>/<slot>
```

`<имя>` — **имя самого пира**: первая группа его GUID (8 hex-символов, `1a2b3c4d` из
`1a2b3c4d-…`); полный GUID — секрет пары и на брокер не попадает. `<slot>` — номер
дыры (0..9). Каждый пир публикует свои слоты на топиках своего имени и подписывается на
слоты пира по `home-proxy/rendezvous/<имя-пира>/+`. Т.е. на
...
session_id, slot, extra_endpoints, signature }`, подписан секретом пары), не текст, поэтому смотреть его сырым `mosquitto_sub`
...
- `client_id` = **его собственное имя** (первая группа GUID);
- `username` = **имя пира, которого он ищет**.
...
Имя — не секрет: знающий его прочитает записи пира (адреса слотов) и сможет занять
его `client_id`. Подделать запись он не сможет — она подписана ключом из полных GUID
пары, без подписи пир её не примет; ключей и векторов в записи нет. Подробно про
устройство ACL, конфиг и проверки — `MosquittoACL.md`.
...
контейнера, порт `8883` с TLS). Подставьте имена одноразовых GUID: `A` — чья запись
лежит, `C` — «посторонний»:
...
$T -i <C> -u <A> -t 'home-proxy/rendezvous/<A>/+'      # видит слоты A: знает его имя
```
